...
/* find out what address the connection was *to* on our end, in case we're
  ...
  int hoplimit = -1;
  socklen_t hoplimitlen = sizeof (hoplimit);
  getsockopt(nnetfd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &hoplimit, &hoplimitlen);

  int mtu = -1;
  socklen_t mtulen = sizeof (mtu);
  getsockopt(nnetfd, IPPROTO_IPV6, IPV6_MTU, &mtu, &mtulen);

  int recvtclass = -1;
  socklen_t recvtclasslen = sizeof (recvtclass);
  getsockopt(nnetfd, IPPROTO_IPV6, IPV6_RECVTCLASS, &recvtclass, &recvtclasslen);
...
/* now check out who it is.  We don't care about mismatched DNS names here,
  ...
  if (x)			/* guilty! */
    bail ("invalid connection to [%s] from %s [%s] %d",
      cp, whozis->name, whozis->addrs[0], z);
  holler ("connect to [%s] from %s [%s] %d HL=%i MTU=%i RECVTCLASS=%i",	/* oh, you're okay.. */
    cp, whozis->name, whozis->addrs[0], z, hoplimit, mtu, recvtclass);
  return (nnetfd);	/* open! */
...
helpme()
{
  ...
  o_holler_stderr = 0;
  holler ("[v1.10]\n\
extra feature by Gabriel Reis and Matheus Sauthier\n\
...
if(strchr(argv[1], ':')) {
  printf("IPv6 address detected %s", argv[1]); printf ("\n"); fflush (stdout);
  want6 = 1;
}
...
/* Debug (("in go: x now %c, optarg %x optind %d", x, optarg, optind)) */
    ...
    switch (x) {
      ...
      case '4':
  if(want6) {
    printf("Don't use the -4 option with a IPv6 address"); printf ("\n"); fflush (stdout);
    return(0);
  }
	want6 = 0;
	break;
